 ...
 *   encaixe = minuto fora da grade de 10
 *
...
/** Minuto fora da grade de 10 em 10 — foi assim que o paciente foi avisado. */
function ehEncaixe(horaConsulta: string | null | undefined): boolean {
  ...
  return Number.isFinite(minuto) && minuto % 10 !== 0;
}
 ...
 *
 * 1. **Encaixe pela hora.** É aritmética sobre um dado que sempre existe, e é o
 *    mesmo cálculo que decidiu o template enviado. Vem primeiro também porque o
 *    log do reenvio manual grava `tipo_consulta` como "Consulta"/"Exame" e perde
 *    o encaixe — sem este degrau, um encaixe reenviado viraria exame.
 * 2. **O tipo que o lembrete anunciou.** `envio_lembrete.payload.tipo_consulta`
 *    é o rótulo que o paciente de fato recebeu. Confiável por chave apesar do
...
export function classificaAtendimento(entrada: {
  ...
  if (anunciado === "exame" || anunciado === "consulta" || anunciado === "encaixe") {
